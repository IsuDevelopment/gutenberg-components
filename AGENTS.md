...
9. **The main package README is the public module catalog.** Every new public component,
   ...
   Everything else is generated by `npm run catalog`.
...
AGENTS.md                # GENERATED — consumer-facing agent guide, shipped in the tarball
catalog.json             # GENERATED — machine-readable module surface, shipped in the tarball
...
- **`npm run catalog`** regenerates `packages/gutenberg/AGENTS.md` and `catalog.json` from
  the colocated READMEs; `npm run build` runs it first. `npm run catalog:check` is the CI
  gate and `tests/catalog.test.ts` the local one. Never hand-edit either generated file —
  the prose at the top of `AGENTS.md` lives in `scripts/agents-preamble.md`. Decision 0011
...
- Keep the public module catalog in `packages/gutenberg/README.md` synchronized: every
  ...
  README.
- Run `npm run catalog` in the same change and commit what it writes. A README edit that
  does not regenerate leaves the shipped agent guide wrong, and CI will fail.
